...
namespace BastionFall.Features.Chat.Shared.Commands
{
        ...
        public static bool TryParseColor(string s, out float4 rgba)
        {
            ...
            if (s[0] == '#')
            {
                ...
                if (hex.Length is 6 or 8 &&
                    uint.TryParse(hex, NumberStyles.HexNumber, null, out var val))
                {
                    float r, g, b, a = 1f;
                    if (hex.Length == 6)
                    {
            ...
            {
                case "red":
                    rgba = new float4(1, 0, 0, 1);
                    return true;
                case "green":
                    rgba = new float4(0, 1, 0, 1);
                    return true;
                case "blue":
                    rgba = new float4(0, 0, 1, 1);
                    return true;
                case "yellow":
                    rgba = new float4(1, 1, 0, 1);
                    return true;
                case "cyan":
                    rgba = new float4(0, 1, 1, 1);
                    return true;
                case "magenta":
                    rgba = new float4(1, 0, 1, 1);
                    return true;
                case "white":
                    rgba = new float4(1, 1, 1, 1);
                    return true;
                case "black":
                    rgba = new float4(0, 0, 0, 1);
                    return true;
                case "orange":
                    rgba = new float4(1, 0.5f, 0, 1);
                    return true;
                case "purple":
                    rgba = new float4(0.5f, 0, 0.5f, 1);
                    return true;
                case "pink":
                    rgba = new float4(1, 0.4f, 0.7f, 1);
                    return true;
            }
            ...
            if (parts.Length is 3 or 4)
            {
                ...
                var scale255 = vals[0] > 1.5f || vals[1] > 1.5f || vals[2] > 1.5f ||
                               (parts.Length == 4 && vals[3] > 1.5f);
